...
x_train, y_train, num_words, sequence_length, tokenizer = texts_to_n_gram_sequences(data.split('\n'))
...
stop_training = callback_of_stop_training(
    lambda logs: logs.get('acc') > 0.9
)

history = model.fit(
    x_train, y_train, epochs=30, shuffle=True
)
